...
def load_model_and_data():
    ...
    # 检查必要文件
    required_files = {
        'dann_model.pth': '训练好的DANN模型权重',
        'bearing_features.csv': '源域特征数据',
        'target_features.npy': '目标域特征数据',
        'target_domain_predictions.csv': '目标域预测结果',
        'tsne_features_after_transfer.npy': '共享特征空间数据'
    }
    ...
    if missing_files:
        ...
        print("\n请先运行第三问的代码生成这些文件！")
        print("!" * 80)
        raise FileNotFoundError("关键数据文件缺失，无法继续执行可解释性分析。")

    # 1. 加载模型
    model = DANN(input_dim=20, num_classes=4)
    ...
    # 3. 加载目标域数据
    X_target = np.load('target_features.npy')
...
def compute_physics_score_for_hypothesis(sample_features, hypothesis_label, feature_columns):
    ...
    if hypothesis_label == 'Outer_Ring_Fault':
    ...
    elif hypothesis_label == 'Inner_Ring_Fault':
    ...
    elif hypothesis_label == 'Ball_Fault':
...
def get_all_class_probabilities(model, sample_features):
    ...
    class_names = ['Ball_Fault', 'Inner_Ring_Fault', 'Normal', 'Outer_Ring_Fault']
